...
BULLET_HIT_SOUND =  pygame.mixer.Sound('./Assets/impacto.wav')
BULLET_FIRE_SOUND = pygame.mixer.Sound('./Assets/disparo.wav')
...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VEL > 0 :   #IZQUIERDA
        yellow.x -= VEL 
    if keys_pressed[pygame.K_d] and yellow.x + VEL - 15 < BORDER.x - SPACESHIP_WIDTH :   #DERECHA
        yellow.x += VEL 
    if keys_pressed[pygame.K_w] and yellow.y - VEL > 0:   #ARRIBA
        yellow.y -= VEL 
    if keys_pressed[pygame.K_s] and yellow.y + VEL < HEIGHT - 15 - SPACESHIP_HEIGHT:   #ABAJO
        yellow.y += VEL 

def red_handle_movement(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x - VEL > BORDER.x + 10  :   #IZQUIERDA
            red.x -= VEL 
    if keys_pressed[pygame.K_RIGHT] and red.x + VEL < WIDTH - SPACESHIP_WIDTH + 15:   #DERECHA
        red.x += VEL 
    if keys_pressed[pygame.K_UP] and red.y - VEL > 0:   #ARRIBA
        red.y -= VEL 
    if keys_pressed[pygame.K_DOWN] and red.y + VEL < HEIGHT - 15 - SPACESHIP_HEIGHT:   #ABAJO
        red.y += VEL     
...
def main():
    ...
    while run:
        ...
        if red_health <= 0:
            winner_text = "Ganador AMARILLO!!"

        if yellow_health <= 0:
            winner_text = "Ganador ROJO!!"
